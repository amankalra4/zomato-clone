...
const Header = ({ color, showLogo }: IHeaderProps) => {
    ...
    return (
        <header>
            <nav data-src={showLogo} className={header} style={{ justifyContent: showLogo ? "space-between" : "flex-end" }}>
                {showLogo && (
                    <div className={imageContainer}>
                        <Image src={image} alt="zomato" width={100} height={25} onClick={() => push("/")} priority />
                    </div>
                )}
                <div className={searchBar}>{showLogo && <Searchbar />}</div>
